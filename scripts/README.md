...
| **verify-content** — `npm run verify:content` / `verify:content:uat` | Prints a side-by-side inventory of everything **published** on an instance: how many items of each type exist in English and in Arabic, which items are missing a translation, and which pages have mismatched addresses between the two languages. Add `--out=dev.json` to save the result, then `--compare=dev.json` on the other instance to prove a content migration copied everything. Those snapshot files are gitignored: they are generated and only meaningful for the migration you are running right now. Reads only; changes nothing. | 🔵 Read-only |
